...
type Expert struct {
	...
	Uuid          string         `gorm:"column:uuid;type:varchar(36);comment:角色ID"`
	Name          string         `gorm:"column:name;index;type:varchar(50);comment:专家姓名"`
	Avatar        string         `gorm:"column:avatar;type:varchar(255);default:'default_expert.png';comment:专家头像"`
	Field         string         `gorm:"column:field;index;type:varchar(100);comment:专业领域"`
	...
	ConsultCount  int            `gorm:"column:consult_count;type:int;default:0;comment:咨询次数"`
	Rating        float64        `gorm:"column:rating;type:decimal(3,2);default:5.00;comment:好评率(0-5)"`
	...
	Status        string         `gorm:"column:status;type:varchar(20);default:'active';comment:专家状态(active/inactive)"`
